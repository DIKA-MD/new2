...
module.exports = (kota) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (!sholat.status) return resolve({ creator: '@DikaOFC', status: false })
        let now = moment(new Date * 1).format('DD-MM-yy')
        let data = sholat.data.find(v => v.tanggal == now)
        if (typeof data == 'undefined') return resolve({ creator: '@DikaOFC', status: false })
        resolve({ creator: '@DikaOFC', status: true, data })
    })
}
